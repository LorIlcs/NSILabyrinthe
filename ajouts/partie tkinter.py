...
def grille_cliquable_sans_doublon(canvas, fenetre, L, H, taille_case=50, ox=50, oy=100):
    ...
    bouton = tk.Button(
        ...
        font=("", 20),
        relief="solid",
        command=lambda: [valider_selection(bouton),recuperer_murs(canvas, ox, oy, taille_case),graphe_mur_selectionners(canvas, ox, oy , taille_case, L,H)]
    )
...
def recuperer_murs(canvas, ox, oy, taille_case):
    ...
    for arete in canvas.find_withtag("verrouille"):
        ...
        gx1 = round((x1 -ox ) / taille_case)
        gy1 = round((y1 - oy) / taille_case)
        gx2 = round((x2 - ox) / taille_case)
        gy2 = round((y2 - oy) / taille_case)
...
def graphe_mur_selectionners(canvas, ox, oy , taille_case, L,H):

    murs = recuperer_murs(canvas, ox , oy, taille_case)
    laby = Labyrinthe(murs=murs ,long =H, larg = L)
    graphe = laby.construit_graphe()
     
    
    print("Nombre de sommets :", len(graphe.sommets()))
